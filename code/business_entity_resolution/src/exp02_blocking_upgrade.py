#!/usr/bin/env python3
"""
Experiment 02: Upgraded Multi-View Blocking.
Objective: Push Blocking Recall Ceiling from ~0.918 to >0.985.

Strategies tested:
  1. Exact normalized-sorted name
  2. Name prefix-4
  3. Double Metaphone (all tokens)
  4. Rare Name Word Inverted Index (tokens len >= 4, doc_freq < 50k)
  5. Postal code match
  6. Address Number + Street Prefix
  7. UNION of all channels
"""
...
def get_rare_tokens(norm_name_str: str) -> set:
    ...
    return {w for w in norm_name_str.split() if len(w) >= 4}

def get_addr_keys(na_str: str) -> set:
    if not na_str: return set()
    nums = get_nums(na_str)
    toks = [w for w in na_str.split() if len(w) >= 4 and not w.isdigit()]
    keys = set()
    if nums and toks:
        # Pair first number with street token
        keys.add(f"{nums[0]}_{toks[0]}")
    return keys

def main():
    print("=" * 70, flush=True)
    print("EXPERIMENT 02: UPGRADED MULTI-VIEW BLOCKING", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"Candidate pool: {len(keep_cand_ids):,} (true={len(true_match_ids):,}, neg={len(neg_sample):,})", flush=True)
    ...
    print(f"\nBuilding indices across {n:,} candidates...", flush=True)
    ...
    for i in range(n):
        for w in get_rare_tokens(nn_v[i]):
            if tok_df[w] < 25000: # Filter out ubiquitous words
                idx_rare_tok[w].append(i)
    idx_rare_tok = dict(idx_rare_tok)
    print(f"  Rare Name Token Index: {len(idx_rare_tok):,} keys", flush=True)
    ...
    # [6] Addr Key (Num + Street word)
    idx_addr_key = defaultdict(list)
    for i in range(n):
        for ak in get_addr_keys(na_v[i]):
            idx_addr_key[ak].append(i)
    idx_addr_key = dict(idx_addr_key)
    print(f"  Address Key Index: {len(idx_addr_key):,} keys", flush=True)

    # 5. Fast Integer-Set Blocking Evaluation on Dev S1
    print("\nRunning lightning-fast blocking evaluation on Dev S1...", flush=True)
    
    cand_id_to_idx = {cid: i for i, cid in enumerate(cids)}
    
    # Pre-map ground truth to integer index sets
    gt_int = {}
    tot_true_pairs = 0
    ...
    print(f"Total True Pairs to Match in Dev Set: {tot_true_pairs:,}", flush=True)
    ...
    for sid, nn, nns, rn, pc, na in zip(s1_ids, s1_nn, s1_nns, s1_rn, s1_pc, s1_na):
        ...
        # 2. Pfx-4 (limit bucket to <= 1000)
        pfx = nn[:4] if len(nn) >= 4 else nn
        s_pfx = set(idx_pfx.get(pfx, [])) if (pfx and len(idx_pfx.get(pfx, [])) <= 1000) else set()
        stats["2. Name Prefix-4"]["pairs"] += len(s_pfx)
        stats["2. Name Prefix-4"]["found"] += len(s_pfx & true_set)
        
        # 3. Phonetic (limit bucket to <= 1000)
        s_phone = set()
        for k in pkeys(rn):
            b = idx_phone.get(k, [])
            if len(b) <= 1000: s_phone.update(b)
        stats["3. Phonetic"]["pairs"] += len(s_phone)
        stats["3. Phonetic"]["found"] += len(s_phone & true_set)
        
        # 4. Rare Tokens (distinctive words, bucket <= 1000)
        s_rare = set()
        for w in get_rare_tokens(nn):
            b = idx_rare_tok.get(w, [])
            if len(b) <= 1000: s_rare.update(b)
        ...
        # 6. Addr Key (bucket <= 500)
        s_ak = set()
        for ak in get_addr_keys(na):
            b = idx_addr_key.get(ak, [])
            if len(b) <= 500: s_ak.update(b)
        stats["6. Addr Num+Street"]["pairs"] += len(s_ak)
        stats["6. Addr Num+Street"]["found"] += len(s_ak & true_set)
        
        # Union
        s_union = s_exact | s_pfx | s_phone | s_rare | s_post | s_ak
        stats["UNION (ALL)"]["pairs"] += len(s_union)
        stats["UNION (ALL)"]["found"] += len(s_union & true_set)

    print(f"Evaluation finished in {time.time()-t_eval_start:.2f}s", flush=True)

    print("\n" + "=" * 75, flush=True)
    print("BLOCKING RECALL BREAKDOWN", flush=True)
    print("=" * 75, flush=True)
    ...
    log_experiment({
        "experiment_id": "EXP-02-BLOCKING",
        "date": "2026-09-25",
        "description": "Upgraded multi-view blocking (Rare token + Addr Num+Street + Phonetic + Prefix + Postal)",

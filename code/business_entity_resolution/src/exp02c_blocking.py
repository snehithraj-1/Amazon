#!/usr/bin/env python3
"""
Experiment 02c: High-Recall Multi-View Blocking with Distinctive Address Token Indexing.
Specifically solves:
- Transliterated / non-Latin names with identical addresses (Devanagari, Kannada, etc.)
- DBA / different trade names at same facility
- Rare address landmark & building names (e.g. 'Ansal Chambers', 'Purbasha Anandlok', 'Indrapuri')
"""
...
# Common address stopwords to ignore for rare address indexing
ADDR_STOPWORDS = {
    ...
    'road', 'cross', 'main', 'extn', 'hno', 'plot', 'shop', 'flat'
}
...
def main():
    print("=" * 75, flush=True)
    print("EXPERIMENT 02c: MULTI-VIEW BLOCKING WITH RARE ADDRESS TOKEN INDEXING", flush=True)
    print("=" * 75, flush=True)
    ...
    # 8. Rare Address Tokens (Landmarks, buildings, specific street names)
    addr_tok_df = Counter()
    for i in range(n):
    ...
    for i in range(n):
        for w in get_rare_addr_tokens(na_v[i]):
            if addr_tok_df[w] <= 600: # Distinctive address tokens only
                idx_rare_addr[w].append(i)
    idx_rare_addr = dict(idx_rare_addr)
    print(f"  Rare Address Token Index: {len(idx_rare_addr):,} keys", flush=True)
    ...
    stats = {
        ...
        "8. Rare Addr Tokens": {"found": 0, "pairs": 0},
    ...
    for sid, nn, nns, rn, pc, na in zip(s1_ids, s1_nn, s1_nns, s1_rn, s1_pc, s1_na):
        ...
        # 8. Rare Address Tokens (distinctive landmarks/buildings/streets, bucket <= 400)
        s_ra = set()
        for w in get_rare_addr_tokens(na):
            b = idx_rare_addr.get(w, [])
            if len(b) <= 400: s_ra.update(b)
        stats["8. Rare Addr Tokens"]["pairs"] += len(s_ra)
        stats["8. Rare Addr Tokens"]["found"] += len(s_ra & true_set)
        
        # Union
        s_union = s_exact | s_pfx | s_phone | s_rare | s_post | s_ak | s_comp | s_ra
    ...
    log_experiment({
        "experiment_id": "EXP-02C-BLOCKING",
        "date": "2026-09-25",
        "description": "8-channel union blocking with Rare Address Landmark Indexing",
